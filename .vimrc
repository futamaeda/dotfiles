...
"インデントつける →　よくわからんので解除中
"filetype indent on  

"無名レジスタに入るデータをクリップボード(*レジスタ)に　→　
set clipboard+=unnamed 

"ノーマルモードとヴィジュアルモードで ; と : の割り当てを逆にする。
noremap ; :
noremap : ;

"押しづらいやつをスペースと併せて押しやすくする。
noremap <Space>h  ^
noremap <Space>l  $
noremap <Space>m  %
...
"現在ディレクトリを%%で表示byTIP41  なんかうまく動いてないけど
cnoremap <expr> %% getcmdtype () == ':' ? expand('%:h').'/' : '%%'

"nとNで移動短縮? いらないかも Leaderがデフォルトだと\なのでつかいづらそう
...
"おすすめはコミットするたび、なのでまた今度。

"ヴィジュアルモードで選択した部分を検索。vim-visual-star-searchと同じことをさせる。もしかして"sレジスタ使っちゃうの？
...
"vimgrepしてquickfixリストから引数リストに引っ張るコマンド
...
set nocompatible
filetype plugin on
"runtime macros/matchit.vim    "matchit.vimを使えるように(まだディレクトリ作ってない)
